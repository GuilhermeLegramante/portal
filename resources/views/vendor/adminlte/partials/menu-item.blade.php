...
<li @if (isset($item['id'])) id="{{ $item['id'] }}" @endif
    ...
    @php
    $allowedMenus = App\Repositories\ClientRepository::getAllowedMenus();
    $allow = false;
    @endphp 

    {{-- PERCORRE OS MENUS PARA VERIFICAR SE ESTÁ LIBERADO --}}
    @foreach ($allowedMenus as $menu)
    @if( ($menu->rota == $item['url']) && ($menu->liberado == 1))
    @php
    $allow = true;
    break;
    @endphp
    ...
    {{-- CASO ESTEJA LIBERADO, EXIBE O MENU --}}
    @if($allow)
